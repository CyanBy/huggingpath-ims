...
function Badge({ children, type }: { children: string; type: 'purple' | 'green' | 'blue' | 'red' | 'gray' }) {
  const className =
    type === 'purple'
  ...
  return (
    <span className={`h-6 px-2.5 rounded-full inline-flex items-center text-xs font-bold ${className}`}>
      {children}
    </span>
  );
}
...
function StatusBadge({ status }: { status: AiStatus }) {
  if (status === '分析完成') return <Badge type="green">分析完成</Badge>;
  if (status === '分析中') return <Badge type="blue">分析中</Badge>;
  if (status === '分析失败') return <Badge type="red">分析失败</Badge>;
  return <Badge type="gray">未分析</Badge>;
}
...
export default function WorkbenchOther() {
  ...
  const [selectedCaseId, setSelectedCaseId] = useState<string>('25AH032093');
  ...
  return (
    <div className="h-[calc(100dvh-64px)] bg-[#0f1014] text-[#f1f3f6] overflow-hidden">
      <div className="h-full min-h-0">
        <section className="min-w-0 overflow-auto p-6 bg-[radial-gradient(circle_at_78%_10%,rgba(143,53,183,0.12),transparent_34%),#0f1014]">
          {!showDetail ? (
              ...
              <div className="rounded-xl border border-white/[0.08] bg-[#202126] overflow-hidden">
                ...
                <table className="w-full table-fixed border-collapse text-sm">
                  ...
                  <tbody>
                    {cases.map((item) => {
                      const showWorkbench =
                        item.aiStatus === '分析中' || item.aiStatus === '分析完成' || item.aiStatus === '分析失败';
                      ...
                      return (
                        <tr key={item.caseId} className="border-b border-white/[0.06] hover:bg-white/[0.025]">
                          ...
                          <td className="h-[58px] px-3.5">
                            <Badge type="purple">{item.part}</Badge>
                          </td>
                          ...
                          <td className="h-[58px] px-3.5">
                            <div className="flex gap-3 text-[#d292f4] text-sm whitespace-nowrap">
                              <button onClick={() => openDetail(item.caseId)}>查看</button>
                              {showJoin && (
                                <button onClick={() => joinWorkbench(item.caseId)}>加入工作台</button>
                              )}
                              {showWorkbench && <button>查看工作台</button>}
            ...
            <div className="rounded-xl border border-white/[0.08] bg-[#202126] p-5 min-h-[610px]">
              <div className="flex items-center justify-between border-b border-white/[0.06] pb-4 mb-5">
                <div>
                  <div className="text-2xl font-extrabold">病例详情</div>
                  <div className="text-xs text-[#64748b] mt-1">
                    当前仅展示病例详情页面占位，具体详情设计后续再展开。
                  </div>
                </div>
                <button
                  onClick={() => setShowDetail(false)}
                  className="h-[38px] px-4 rounded-md border border-white/[0.08] bg-[#202126] text-[#e2e8f0] text-sm"
                >
                  返回病例列表
                </button>
              </div>

              <div className="h-[460px] rounded-xl border border-dashed border-[#8f35b7]/50 bg-[#8f35b7]/10 flex items-center justify-center text-center text-[#d292f4] leading-8">
                病例详情页面
                <br />
                当前病例：{selectedCaseId}
                <br />
