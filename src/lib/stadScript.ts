...
const cohortReply: StadReply = {
  thinking: '读取 tables/cohort_manifest.csv（375 例病例清单与分组）；\n核对 cohort_label_inventory.md 的 37 字段标签审计（泄漏/混杂/批次已分类）；\n确认特征矩阵规模：951 项 TME 特征，无监督质控后 851 项合格。',
...
const diffReply: StadReply = {
  thinking: '对 851 项合格特征逐一做 Mann-Whitney U 检验 + Cohen\'s d 效应量；\nBH-FDR 多重校正，按 |d| 排序取 Top 10；\n绘制 Top 10 特征的病例级小提琴图并核验。',
...
const classifyReply: StadReply = {
  thinking: '分层留出：训练集 263 例 / 独立留出集 112 例（seed=42）；\n训练集内按 |d| 选 top-10 特征，L2 逻辑回归；\n独立留出集评估 + 5 折交叉验证，附混杂检查。',
...
const tsneReply: StadReply = {
  thinking: '匹配 cBioPortal 分子亚型标签，328/375 例纳入（CIN 192 / MSI 59 / GS 44 / EBV 27 / POLE 6）；\n845 个有效特征联合 t-SNE 降维，按亚型着色；\nKruskal-Wallis + η² 排序亚型区分度 Top10 特征。',
...
const survivalReply: StadReply = {
  thinking: '5 折交叉内筛选特征，Cox 比例风险模型（penalizer 1.0）；\n按风险评分中位数分层，绘制 KM 曲线；\n200 次 bootstrap 校验；终点 OS（任何原因死亡），删失规则见产物数据。',
