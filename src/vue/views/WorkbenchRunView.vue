...
const activeResultName = computed(() => runs.value.find((run) => run.modelId === 'mod-tme')?.name || 'TME Analyzer')
...
<template>
  <div v-if="task" class="workbench-shell">
    ...
    <aside class="workbench-results">
      <header class="results-header"><div class="flex items-start justify-between gap-3"><div class="min-w-0"><h2>分析结果</h2><p class="truncate" :title="selectedObject?.name">{{ selectedObject?.name }}</p></div><span>示例结果</span></div><div class="mt-3 flex items-center justify-between border-t border-white/[0.07] pt-3"><b>{{ activeResultName }}</b><small>263 维特征</small></div></header>
      <nav class="result-tabs"><button :class="resultTab === 'quantitative' && 'active'" @click="resultTab = 'quantitative'">TME 定量</button><button :class="resultTab === 'features' && 'active'" @click="resultTab = 'features'">特征分析</button><button :class="resultTab === 'report' && 'active'" @click="resultTab = 'report'">报告-Agent</button></nav>
      ...
      <div v-else-if="resultTab === 'features'" class="results-scroll feature-results"><div class="feature-summary"><div><span>特征数</span><b>263</b></div><div><span>细胞</span><b>151,498</b></div><div><span>ROI</span><b>80</b></div><div><span>耗时</span><b>21s</b></div></div><div class="feature-filters"><select v-model="featureCategory"><option v-for="category in featureCategories" :key="category">{{ category }}</option></select><label><Search :size="15" /><input v-model="featureSearch" placeholder="搜索特征" /></label></div><div class="feature-list"><button v-for="feature in filteredFeatures" :key="feature.id" :class="selectedFeatureId === feature.id && 'active'" @click="selectFeature(feature)"><span class="min-w-0"><b :title="feature.name"><i v-if="feature.spatial" />{{ feature.name }}</b><small>{{ feature.category }}</small></span><strong>{{ feature.value }} <small>{{ feature.unit }}</small></strong><em v-if="feature.spatial">空间</em></button></div></div>
      <div v-else class="results-scroll report-panel"><div class="report-status"><FileText :size="20" /><div><b>TME 分析报告</b><p>根据当前 WSI 的定量和特征结果生成。</p></div></div><template v-if="reportGenerated"><section><h3>结果摘要</h3><p>当前组织区域以 Stroma 为主，占组织面积约 88%。细胞核构成中 Stromal nucleus 数量最高，空间特征结果建议结合 ROI 复核。</p></section><section><h3>重点指标</h3><ul><li>组织面积：38.9 mm²</li><li>细胞核总数：178,055</li><li>空间特征：5 项可查看</li></ul></section></template><button class="btn-primary w-full" @click="reportGenerated = true"><FileText :size="16" />{{ reportGenerated ? '重新生成报告' : '生成报告草稿' }}</button></div>
    </aside>

    <Teleport to="body"><div v-if="hoveredProgressId" class="pointer-events-none fixed z-[220] w-[292px] rounded-md border border-white/[0.12] bg-[#111217]/98 p-3 shadow-[0_18px_48px_rgba(0,0,0,.55)]" :style="{ left: `${progressPopover.left}px`, top: `${progressPopover.top}px` }"><div class="mb-2 text-xs font-semibold text-[#cbd5e1]">模型分析进度</div><div v-for="run in getObjectRuns(task, hoveredProgressId)" :key="run.id" class="mb-2.5 last:mb-0"><div class="flex items-center justify-between gap-3 text-xs"><span class="min-w-0 truncate text-[#e2e8f0]">{{ run.name }}</span><span class="shrink-0 text-[#94a3b8]">{{ run.status }} · {{ runProgress(run) }}%</span></div><div class="mt-1.5 h-1.5 overflow-hidden rounded-full bg-[#353740]"><div class="h-full rounded-full bg-[#9c36c7]" :style="{ width: `${runProgress(run)}%` }" /></div></div></div></Teleport>
  </div>
