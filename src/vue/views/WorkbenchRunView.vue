...
const showCaseContext = computed(() => (route.query.historyScope === 'case' || task.value?.objectType === 'Case') && Boolean(selectedCase.value) && !isMultiCase.value)

/** 多 Case 任务：切片按 Case 分组，避免不同 Case 的数据混在一起 */
const caseGroups = computed(() => {
  const groups = new Map<string, { key: string; caseName: string; items: AnalysisTaskObjectRecord[] }>()
  for (const item of filteredObjects.value) {
    const key = item.caseId || item.caseName || '__none__'
    if (!groups.has(key)) groups.set(key, { key, caseName: item.caseName || item.caseId || '未归属 Case', items: [] })
    groups.get(key)!.items.push(item)
  }
  return [...groups.values()]
})
const isMultiCase = computed(() => caseGroups.value.length > 1)
/** 文件夹式收起：收起的 Case 组 id 列表 */
const collapsedCaseGroups = ref<string[]>([])
function toggleCaseGroup(caseId: string) {
  collapsedCaseGroups.value = collapsedCaseGroups.value.includes(caseId)
    ? collapsedCaseGroups.value.filter((id) => id !== caseId)
    : [...collapsedCaseGroups.value, caseId]
}
/** Case 临床信息面板（组头 ⓘ 按钮展开，与收起互不干扰） */
const expandedCaseInfoId = ref<string | null>(null)
function toggleCaseInfo(caseId: string) {
  expandedCaseInfoId.value = expandedCaseInfoId.value === caseId ? null : caseId
}
function caseRecordOf(key: string) {
  return cases.value.find((item) => item.id === key) || null
}
  ...
  <div v-if="task" class="workbench-shell">
    <aside class="workbench-left">
      <div class="border-b border-white/[0.08] p-3">
        ...
        <div class="mt-3 flex items-start justify-between gap-3"><h1 class="min-w-0 text-lg font-semibold"><span v-marquee class="title-marquee" :title="getTaskDisplayName(task)"><span class="title-marquee-track"><span class="pr-8">{{ getTaskDisplayName(task) }}</span><span class="pr-8" aria-hidden="true">{{ getTaskDisplayName(task) }}</span></span></span></h1><span :class="['shrink-0 rounded-md border px-2 py-1 text-xs', statusClass(task.status)]">{{ task.status }}</span></div>
      </div>
      <section v-if="showCaseContext && selectedCase && !isMultiCase" class="case-context">
      ...
      <div class="min-h-0 flex-1 space-y-2 overflow-y-auto p-3" @scroll.passive="hideFileName">
        <template v-for="group in caseGroups" :key="group.key">
          <div v-if="isMultiCase" class="case-group">
            <button class="case-group-toggle" :aria-expanded="!collapsedCaseGroups.includes(group.key)" @click="toggleCaseGroup(group.key)">
              <ChevronDown v-if="!collapsedCaseGroups.includes(group.key)" :size="14" />
              <ChevronRight v-else :size="14" />
              <b class="min-w-0 flex-1 truncate">{{ group.caseName }}</b>
              <small class="shrink-0">{{ caseRecordOf(group.key) ? `${caseRecordOf(group.key)!.patientCode} · ${caseRecordOf(group.key)!.sex}` : '未归属' }} · {{ group.items.length }} 张</small>
              <span class="case-info-btn" title="查看 Case 临床信息" @click.stop="toggleCaseInfo(group.key)"><Info :size="13" /></span>
            </button>
            <div v-if="expandedCaseInfoId === group.key && caseRecordOf(group.key)" class="case-context-body mb-2">
              <div><span>脱敏患者</span><b>{{ caseRecordOf(group.key)!.patientCode }}</b></div>
              <div><span>年龄 / 性别</span><b>{{ caseRecordOf(group.key)!.age ?? '未知' }} / {{ caseRecordOf(group.key)!.sex }}</b></div>
              <div><span>取材</span><b>{{ getPathologySiteLabel(caseRecordOf(group.key)!.site) }} · {{ getSamplingMethodLabel(caseRecordOf(group.key)!.samplingMethod) }}</b></div>
              <div><span>收样日期</span><b>{{ caseRecordOf(group.key)!.receivedAt }}</b></div>
              <div class="wide"><span>临床诊断</span><b>{{ caseRecordOf(group.key)!.diagnosis }}</b></div>
              <button @click="router.push(`/workbench/cases/${group.key}`)">查看完整 Case 详情</button>
            </div>
          </div>
        <template v-if="!collapsedCaseGroups.includes(group.key)">
        <article v-for="item in group.items" :key="item.id" :class="['rounded-lg border p-3 transition', selectedObject?.id === item.id ? 'border-[#8f35b7] bg-[#8f35b7]/7' : 'border-white/[0.08] bg-[#17181d] hover:border-white/[0.14]']" @click="selectObject(item.id)">
      ...
      <section v-if="selectedObject" :class="['model-selection', modelSelectionLocked && 'locked']"><div class="flex items-center justify-between gap-2"><h3>分析模型</h3><span class="shrink-0 text-[10px] text-[#748095]">{{ modelSelectionLocked ? '已锁定 · ' : '' }}{{ task.objects.filter(item => item.modelIds.length).length }}/{{ task.objects.length }} 张 · {{ taskProgress }}%</span></div><div class="mt-2 flex flex-wrap gap-1.5"><button v-for="model in visibleAnalysisModels" :key="model.id" :disabled="modelSelectionLocked || !isModelCompatible(model, selectedObject.stain)" :class="['model-chip', selectedObject.modelIds.includes(model.id) && 'active', !isModelCompatible(model, selectedObject.stain) && 'na']" :title="model.desc" @click="toggleModel(model.id)">{{ model.name }}<i>{{ !isModelCompatible(model, selectedObject.stain) ? '不适用' : selectedObject.modelIds.includes(model.id) ? '已选' : '可选' }}</i></button></div></section>
      <div class="border-t border-white/[0.08] p-3"><button v-if="['正在分析', '排队中'].includes(task.status)" class="flex h-10 w-full items-center justify-center gap-2 rounded-md border border-[#eab65b]/40 bg-[#eab65b]/10 font-medium text-[#f4c577]" @click="stopAll"><Square :size="14" class="fill-current" />停止全部</button><button v-else :disabled="!task.models.length" class="btn-primary h-10 w-full disabled:opacity-40" @click="startOrRestart"><Play :size="16" />{{ task.status === '待分析' ? '开始分析' : '重新分析任务' }}</button></div>
    </aside>
...
.result-history-toggle{display:flex;width:100%;height:39px;align-items:center;justify-content:space-between;margin-top:11px;border:1px solid rgb(255 255 255 / .07);border-radius:6px;background:#191a20;padding:0 9px;text-align:left}.result-history-toggle:hover{border-color:rgb(143 53 183 / .38);background:rgb(143 53 183 / .07)}.result-history-toggle>span{display:flex;min-width:0;align-items:center;gap:6px}.result-history-toggle svg{flex:none;color:#b86fd7}.result-history-toggle b{color:#cbd5e1;font-size:12.5px}.result-history-toggle small{overflow:hidden;color:#718096;font-size:11px;text-overflow:ellipsis;white-space:nowrap}.result-history-toggle>svg{color:#64748b;transition:transform .15s}.result-history-toggle>svg.open{transform:rotate(180deg)}.analysis-history{max-height:300px;flex:none;overflow-y:auto;border-bottom:1px solid rgb(255 255 255 / .08);background:#17181d;padding:8px 10px}.analysis-history>button{display:grid;width:100%;grid-template-columns:18px minmax(0,1fr);gap:8px;padding:9px 6px;text-align:left}.analysis-history>button>i{position:relative;display:flex;justify-content:center}.analysis-history>button>i::after{position:absolute;top:13px;bottom:-14px;width:1px;background:#343642;content:''}.analysis-history>button:last-child>i::after{display:none}.analysis-history>button>i>span{position:relative;z-index:1;width:8px;height:8px;margin-top:4px;border:2px solid #5e6270;border-radius:50%;background:#17181d}.analysis-history>button.active>i>span{border-color:#c46ce6;background:#9c36c7;box-shadow:0 0 0 3px rgb(156 54 199 / .12)}.analysis-history>button>div{min-width:0}.analysis-history>button>div>div{display:flex;align-items:center;justify-content:space-between;gap:7px}.analysis-history b{overflow:hidden;color:#c3ccd8;font-size:14px;text-overflow:ellipsis;white-space:nowrap}.analysis-history em{flex:none;border:1px solid;border-radius:10px;padding:4px 9px;font-size:11.5px;font-style:normal}.analysis-history p{margin-top:4px;overflow:hidden;color:#8d99aa;font-size:13px;text-overflow:ellipsis;white-space:nowrap}.analysis-history small{display:block;margin-top:4px;overflow:hidden;color:#66748a;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:12px;text-overflow:ellipsis;white-space:nowrap}.analysis-history>button.active>div{border-radius:5px;background:rgb(143 53 183 / .08);padding:5px 6px;margin:-5px -6px}.analysis-history>button:hover>div{color:#e8b8f8}
