...
const filtered = computed(() => {
  ...
  return wsis.value.filter((item) => !query || `${item.fileName} ${item.site} ${getPathologySiteLabel(item.site)} ${item.stain} ${item.boundCase}`.toLowerCase().includes(query))
})
...
const previewIndex = computed(() => filtered.value.findIndex((item) => item.id === previewId.value))
const preview = computed(() => previewIndex.value >= 0 ? filtered.value[previewIndex.value] : null)
...
function toggleAll() {
  if (allSelected.value) selectedIds.value = selectedIds.value.filter((id) => !filtered.value.some((item) => item.id === id))
  else selectedIds.value = [...new Set([...selectedIds.value, ...filtered.value.map((item) => item.id)])]
}
...
function primaryAction(item: WorkspaceWsi) {
  const current = currentAnalyses.value.get(item.id)
  if (current && ['待分析', '排队中', '正在分析'].includes(current.status)) return router.push(`/workbench/run/${current.taskId}`)
  const task = createTaskFromWsi({ id: item.id, fileName: item.fileName, size: item.size, organPart: item.site, stain: item.stain, boundCase: item.boundCase })
  router.push(`/workbench/run/${task.id}`)
}
function startSelected() {
  if (!selected.value.length) return
  const task = createTaskFromWsis({ wsis: selected.value.map((item) => ({ id: item.id, fileName: item.fileName, size: item.size, organPart: item.site, stain: item.stain, boundCase: item.boundCase })) })
  router.push(`/workbench/run/${task.id}`)
}
...
function movePreview(direction: -1 | 1) {
  ...
  if (next >= 0 && next < filtered.value.length) previewId.value = filtered.value[next].id
}
...
function basePending(name: string, size: number, source: PendingRow['source']): PendingRow {
  return { id: `wsi-${crypto.randomUUID()}`, fileName: name, size: fileSize(size), site: '', samplingMethod: '', stain: inferStain(name), boundCase: '未绑定', uploadedAt: new Date().toISOString().slice(0, 10), source }
}
...
<template>
  <div class="min-h-[calc(100dvh-64px)] px-4 py-5 lg:px-6">
    ...
    <div class="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-4"><div v-for="item in [['WSI 总数',wsis.length],['已绑定 Case',wsis.filter(w=>w.boundCase!=='未绑定').length],['待绑定',wsis.filter(w=>w.boundCase==='未绑定').length],['模拟存储占用',`${storage} GB`]]" :key="String(item[0])" class="rounded-lg border border-white/[0.08] bg-[#202126] p-4"><small class="text-[#64748b]">{{ item[0] }}</small><div class="mt-2 text-2xl font-bold">{{ item[1] }}</div></div></div>
    <section class="overflow-hidden rounded-lg border border-white/[0.08] bg-[#202126]"><header class="flex flex-wrap items-center justify-between gap-3 border-b border-white/[0.06] p-4"><div><h2 class="font-semibold">WSI 文件列表</h2><p class="mt-1 text-xs">选择后可批量开始分析，文件名和查看操作均打开切片预览。</p></div><div class="flex flex-wrap gap-2"><label class="flex h-9 items-center gap-2 rounded-md border border-white/[0.08] bg-[#17181d] px-3"><Search :size="15" class="text-[#64748b]" /><input v-model="keyword" class="w-[240px] bg-transparent text-sm outline-none" placeholder="搜索 WSI 文件 / Case" /></label><button :disabled="!selected.length" class="btn-primary h-9 disabled:cursor-not-allowed disabled:opacity-40" @click="startSelected">分析已选 WSI（{{ selected.length }}）</button></div></header><div class="overflow-x-auto"><table class="w-full min-w-[1160px] text-sm"><thead class="bg-[#252730]"><tr><th class="w-12"><input type="checkbox" :checked="allSelected" @change="toggleAll" /></th><th>文件名</th><th>缩略图</th><th>文件大小</th><th>取材部位</th><th>取材方式</th><th>染色</th><th>绑定 Case</th><th>当前分析</th><th>分析次数</th><th>操作</th></tr></thead><tbody><tr v-for="item in filtered" :key="item.id" :class="['border-t border-white/[0.06] hover:bg-white/[0.025]', selectedIds.includes(item.id) && 'bg-[#8f35b7]/10']" @click="rowClick($event,item.id)"><td><input type="checkbox" :checked="selectedIds.includes(item.id)" @change="toggleRow(item.id)" /></td><td><button class="font-mono text-left text-[#d292f4]" @click="showPreview(item)">{{ item.fileName }}</button></td><td><button class="h-10 w-16 overflow-hidden rounded border border-white/[0.08]" @click="showPreview(item)"><img src="/wsi-demo.jpg" alt="WSI 缩略图" class="h-full w-full object-cover" /></button></td><td>{{ item.size }}</td><td>{{ getPathologySiteLabel(item.site) }}</td><td>{{ getSamplingMethodLabel(item.samplingMethod) }}</td><td>{{ item.stain }}</td><td><button :disabled="item.boundCase==='未绑定'" :class="item.boundCase==='未绑定' ? 'text-[#64748b]' : 'text-[#d292f4] hover:underline'" @click="openCase(item)">{{ item.boundCase }}</button></td><td><span class="rounded-md border border-white/[0.08] bg-white/[0.03] px-2 py-1 text-xs">{{ statusText(item) }}</span></td><td>{{ countWsiSuccessfulAnalyses(item.id,tasks) }} 次</td><td><div class="flex gap-3 whitespace-nowrap"><button class="text-[#d292f4]" @click="showPreview(item)">查看</button><button class="text-[#d292f4]" @click="primaryAction(item)">{{ currentAnalyses.get(item.id) ? '查看任务' : '开始分析' }}</button><button class="text-[#ff9c9c]" @click="requestDelete(item)">删除</button></div></td></tr></tbody></table></div></section>

    <Teleport to="body">
      <div v-if="preview" class="fixed inset-0 z-[130] grid place-items-center bg-black/80 px-4" @click.self="previewId=null"><section class="w-full max-w-[900px] overflow-hidden rounded-lg border border-white/[0.10] bg-[#202126]"><header class="flex items-center justify-between border-b border-white/[0.08] p-4"><div><h2 class="font-semibold">{{ preview.fileName }}</h2><p class="mt-1 text-xs">{{ previewIndex+1 }} / {{ filtered.length }} · {{ getPathologySiteLabel(preview.site) }} · {{ preview.stain }}</p></div><button class="icon-button" @click="previewId=null"><X :size="19" /></button></header><div class="relative grid min-h-[480px] place-items-center bg-[#0f1014] p-10"><img src="/wsi-demo.jpg" alt="WSI 预览" class="max-h-[440px] max-w-full object-contain" /><button class="nav-arrow left-4" :disabled="previewIndex<=0" @click="movePreview(-1)"><ChevronLeft :size="24" /></button><button class="nav-arrow right-4" :disabled="previewIndex>=filtered.length-1" @click="movePreview(1)"><ChevronRight :size="24" /></button></div></section></div>
      <div v-if="deleteTarget" class="fixed inset-0 z-[140] grid place-items-center bg-black/75 px-4"><section class="w-full max-w-[520px] rounded-lg border border-white/[0.10] bg-[#202126]"><header class="flex items-center gap-3 border-b border-white/[0.07] p-5"><span class="grid h-9 w-9 place-items-center rounded-lg bg-[#ff9c9c]/10 text-[#ffb4b4]"><AlertTriangle :size="18" /></span><h2 class="font-semibold">删除 {{ deleteTarget.fileName }}</h2></header><div class="p-5"><p class="text-sm leading-6">{{ hasWsiRelationship(deleteTarget.id) ? '该 WSI 存在 Case 或研究项目关联，请选择删除范围。' : '该 WSI 没有关联对象，删除后将不再出现在列表中。' }}</p><div v-if="hasWsiRelationship(deleteTarget.id)" class="mt-4 grid gap-2"><label class="scope"><input v-model="deleteScope" type="radio" value="wsi" />仅删除当前 WSI</label><label class="scope"><input v-model="deleteScope" type="radio" value="chain" />删除关联链路（影响 {{ getWsiDeletionImpact(deleteTarget.id).wsiIds.length }} 张 WSI、{{ getWsiDeletionImpact(deleteTarget.id).caseIds.length }} 个 Case、{{ getWsiDeletionImpact(deleteTarget.id).projects.length }} 个项目）</label></div></div><footer class="flex justify-end gap-2 border-t border-white/[0.07] p-4"><button class="btn-secondary" @click="deleteTarget=null">取消</button><button class="rounded-md bg-[#b64250] px-4 text-sm text-white" @click="confirmDelete">确认删除</button></footer></section></div>
    </Teleport>
