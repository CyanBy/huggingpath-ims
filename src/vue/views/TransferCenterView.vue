...
import { Ban, CheckCircle2, CircleAlert, Clock3, CloudUpload, FileImage, RefreshCw, Search, Sparkles, Trash2 } from '@lucide/vue'
...
/** 异常行入口：带着传输上下文去问 AI（新对话 + 自动提问） */
function askAgent(item: UploadTransferItem) {
  router.push({
    path: '/assistant/chat',
    query: { attachKind: 'transfer', attachId: item.id, attachLabel: item.wsi.fileName, q: '这个传输为什么失败', entry: '传输队列' },
  })
}
