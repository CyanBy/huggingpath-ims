    ...
    <section class="records-panel">
      ...
      <div v-else class="records-table-wrap">
        <table class="transfer-table">
          <thead><tr><th>文件</th><th>取材信息</th><th>绑定 Case</th><th>状态与进度</th><th>创建时间</th><th>结束时间</th><th>操作</th></tr></thead>
          <tbody>
            <tr v-for="item in filtered" :key="item.id">
              <td><div class="file-cell"><span><FileImage :size="18" /></span><div><b :title="item.wsi.fileName">{{ item.wsi.fileName }}</b><small>{{ item.wsi.size }} · {{ item.wsi.source === 'dicom-series' ? 'DICOM 序列' : 'WSI 文件' }}</small></div></div></td>
              ...
              <td><div class="table-progress"><div class="status-line"><em :class="tone(item)">{{ item.status }}</em><strong>{{ item.progress }}%</strong></div><div><i :class="tone(item)" :style="{width:`${item.progress}%`}" /></div><small v-if="item.status==='文件处理中'">正在校验文件并生成缩略图</small></div></td>
              <td><time>{{ formatTime(item.createdAt) }}</time></td>
              <td><time :class="!item.completedAt&&'muted'">{{ formatTime(item.completedAt) }}</time></td>
              <td><div class="table-actions"><button v-if="['等待上传','上传中','文件处理中'].includes(item.status)" class="cancel" @click="cancelTransfer(item.id)"><Ban :size="14" />取消</button><button v-else-if="['上传失败','已取消'].includes(item.status)" class="retry" @click="retryTransfer(item.id)"><RefreshCw :size="14" />重试</button><button v-if="!['等待上传','上传中','文件处理中'].includes(item.status)" class="remove" @click="removeTransfer(item.id)"><Trash2 :size="14" />移除</button></div></td>
            </tr>
          </tbody>
        </table>
      </div>
...
.records-table-wrap{overflow-x:auto}.transfer-table{width:100%;min-width:1120px;border-collapse:collapse}.transfer-table th{height:42px;background:#252730;padding:0 14px;color:#9aa7ba;font-size:11px;font-weight:600;text-align:left}.transfer-table td{height:72px;border-top:1px solid rgb(255 255 255 / .065);padding:10px 14px;color:#cbd5e1;font-size:11px;vertical-align:middle}.transfer-table tbody tr{transition:background .14s}.transfer-table tbody tr:hover{background:rgb(255 255 255 / .025)}.transfer-table th:first-child{min-width:260px}.transfer-table th:nth-child(2){width:130px}.transfer-table th:nth-child(3){width:170px}.transfer-table th:nth-child(4){width:300px}.transfer-table th:nth-child(5),.transfer-table th:nth-child(6){width:155px}.transfer-table th:last-child{width:125px}.file-cell{display:flex;min-width:0;align-items:center;gap:10px}.file-cell>span{display:grid;width:36px;height:36px;flex:none;place-items:center;border-radius:7px;background:#292b33;color:#9eaabd}.file-cell>div{min-width:0}.file-cell b,.file-cell small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.file-cell b{max-width:300px;color:#d8dee9;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:11px}.file-cell small{margin-top:4px;color:#64748b;font-size:9px}.cell-primary,.cell-secondary{display:block}.cell-primary{color:#cbd5e1;font-size:11px}.cell-secondary{margin-top:4px;color:#748196;font-size:9px}.case-value{color:#d292f4;font-family:'JetBrains Mono',ui-monospace,monospace;font-size:10px}.case-value.unbound{color:#657287;font-family:inherit}.table-progress{min-width:220px}.status-line{display:flex;align-items:center;justify-content:space-between;gap:8px}.status-line em{border-radius:9px;padding:3px 7px;font-size:9px;font-style:normal}.status-line em.active{background:rgb(143 53 183 / .14);color:#d292f4}.status-line em.done{background:rgb(34 197 94 / .1);color:#65d6a0}.status-line em.failed{background:rgb(239 68 68 / .1);color:#ff8e95}.status-line strong{color:#91a0b4;font-size:9px;font-weight:500}.table-progress>div:nth-child(2){height:5px;margin-top:8px;overflow:hidden;border-radius:3px;background:#30323a}.table-progress i{display:block;height:100%;border-radius:3px;background:#a64ed0}.table-progress i.done{background:#34bd83}.table-progress i.failed{background:#ef535a}.table-progress>small{display:block;margin-top:5px;color:#718096;font-size:8.5px}.transfer-table time{color:#8794a7;font-size:9.5px;white-space:nowrap}.transfer-table time.muted{color:#4f5a6a}.table-actions{display:flex;align-items:center;gap:3px}.table-actions button{display:flex;height:31px;align-items:center;gap:4px;border-radius:5px;padding:0 7px;color:#8d9aad;font-size:9.5px}.table-actions button:hover{background:rgb(255 255 255 / .05);color:#e2e8f0}.table-actions .retry{color:#d292f4}.table-actions .remove{color:#ef8d94}@media(max-width:800px){.records-table-wrap{margin:0}.transfer-table{min-width:1040px}}
</style>
