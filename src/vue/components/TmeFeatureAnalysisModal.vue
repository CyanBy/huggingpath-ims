...
<template>
  <Teleport to="body">
    <div class="tme-modal-backdrop" @click.self="emit('close')">
      <section class="tme-modal" role="dialog" aria-modal="true" aria-label="特征分析">
        <header class="tme-modal-header">
          <div><h2><Sparkles :size="17" />特征分析 · 951 维向量特征</h2><p>计算细胞、组织和空间关系特征，并从已分析切片生成批量统计。</p></div>
        ...
        <div class="tme-modal-body">
          <aside class="tme-sidebar">
            ...
            <div class="full-mode-badge"><Sparkles :size="14" /><span>特征范围</span><b>951 个特征</b></div>
            <button class="run-button" :disabled="running || !selectedFeatureIds.size" @click="startAnalysis"><Play :size="15" />{{ running ? `分析中 ${progress}%` : '开始特征分析' }}</button>
            <button class="rerun-button" :disabled="running" @click="clearAndRerun"><RotateCcw :size="14" />清理结果重跑</button>

            <dl class="run-summary">
              <div><dt>模式</dt><dd>951 维</dd></div>
              ...
              <div><dt>计算</dt><dd class="full">标准计算</dd></div>
            </dl>
          ...
          <main class="tme-results">
            <nav class="tme-tabs"><button :class="resultTab === 'heatmap' && 'active'" @click="resultTab = 'heatmap'"><Grid3X3 :size="14" />{{ analyzedObjects.length > 1 ? '热力图 + 统计' : '统计分布' }}</button><button :class="resultTab === 'table' && 'active'" @click="resultTab = 'table'"><Table2 :size="14" />完整特征表（{{ selectedFeatureIds.size }}）</button><button :class="resultTab === 'spatial' && 'active'" @click="resultTab = 'spatial'"><Layers :size="14" />空间热力图</button><span>已分析切片：{{ hasResult ? analyzedObjects.length : 0 }} · 951 维特征</span></nav>
            ...
            <template v-else-if="resultTab === 'heatmap'">
              <section v-if="analyzedObjects.length > 1" class="heatmap-section">
...
/* 特征面板承担密集数据阅读，统一提高文字和控件字号，避免只放大标题后表格仍难辨认。 */
.run-summary .full{color:#90a7f4}
.tme-modal-body{grid-template-columns:330px minmax(0,1fr)}.tme-modal-header h2{font-size:18px}.tme-modal-header p{font-size:13px}.current-slide span{font-size:13px}.current-slide b{font-size:15px}.mode-switch button{font-size:13px}.run-button,.rerun-button{font-size:14px}.run-summary>div,.feature-selection-title{font-size:13.5px}.run-summary small{font-size:11.5px}.feature-search input,.feature-filter-row select,.feature-filter-row label{font-size:13.5px}.quick-actions button{font-size:12.5px}.feature-options b{font-size:12.5px}.feature-options small{font-size:11.5px}.feature-options button{min-height:44px}.tme-tabs button{font-size:14px}.tme-tabs>span,.analysis-progress span{font-size:12.5px}.heatmap-section h3,.full-table-section h3,.statistics-section h3{font-size:15px}.heatmap-section p,.full-table-section p,.heatmap-section header>span,.full-table-section header>span,.statistics-section header span{font-size:12.5px}.table-scroll table{font-size:13px}.table-scroll th,.table-scroll td{height:42px}.spatial-section>aside h3{font-size:15px}.spatial-section>aside>p,.spatial-section>aside span,.spatial-viewer header span,.spatial-viewer label{font-size:12.5px}.spatial-section>aside small{font-size:11.5px}.spatial-viewer header b{font-size:15px}.spatial-legend{font-size:11.5px}
