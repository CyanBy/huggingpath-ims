      ...
      <section class="tme-modal" role="dialog" aria-modal="true" aria-label="特征分析">
        ...
        <div class="tme-modal-body">
          ...
          <main class="tme-results">
            <nav class="tme-tabs"><button :class="resultTab === 'heatmap' && 'active'" @click="resultTab = 'heatmap'"><Grid3X3 :size="14" />热力图 + 统计</button><button :class="resultTab === 'table' && 'active'" @click="resultTab = 'table'"><Table2 :size="14" />完整特征表（{{ selectedFeatureIds.size }}）</button><button :class="resultTab === 'spatial' && 'active'" @click="resultTab = 'spatial'"><Layers :size="14" />空间热力图</button><span>已分析切片：{{ hasResult ? analyzedObjects.length : 0 }} · 951 维特征</span></nav>
