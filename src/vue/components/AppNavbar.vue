...
import { ChevronDown, Download, LogOut, Menu, Package, Plug, Search, ShieldCheck, UserCircle, X } from '@lucide/vue'
    ...
    <div v-if="uploadToolOpen" class="fixed inset-0 z-[140] grid place-items-center bg-black/70 px-4 backdrop-blur-sm" @click.self="uploadToolOpen = false">
      <section class="w-full max-w-[460px] overflow-hidden rounded-lg border border-white/[0.12] bg-[#202126] shadow-2xl" role="dialog" aria-modal="true">
        ...
        <div class="p-5">
          ...
          <div class="flex justify-end gap-2"><button class="btn-secondary" @click="uploadToolOpen = false">取消</button><button disabled class="btn-primary cursor-not-allowed opacity-50"><Download :size="16" />安装包暂不可用</button></div>
        </div>
      </section>
    </div>
  </Teleport>
